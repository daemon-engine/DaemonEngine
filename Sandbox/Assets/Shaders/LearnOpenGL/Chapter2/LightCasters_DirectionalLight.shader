...
void main()
{
    ...
    vec3 ambient = _DirectionalLight.ambient * texture(_Material.diffuse, TexCoords).rgb;

    float diff = max(dot(norm, lightDir), 0.0);
    vec3 diffuse = _DirectionalLight.diffuse * diff * texture(_Material.diffuse, TexCoords).rgb;

    vec3 viewDir = normalize(_ViewPos - FragPos);
    vec3 reflectDir = reflect(-lightDir, norm);
    float spec = pow(max(dot(viewDir, reflectDir), 0.0), _Material.shininess);
    vec3 specular = _DirectionalLight.specular * spec * texture(_Material.specular, TexCoords).rgb;
